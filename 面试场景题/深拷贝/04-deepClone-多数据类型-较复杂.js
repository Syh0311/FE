/**
https://juejin.cn/post/6844904046692679693#heading-34

1. 区分对象tag
2. 克隆对象通过，构造函数 或者 原型链
3. Array或者Object的克隆可能出现循环引用问题，通过weakMap解决
4. 递归处理Array或Object
5. 获取对象属性
6. 克隆函数 【先判断是箭头函数还是普通函数】
*/
...
function deepClone(target, isFull = true, cache = new WeakMap(), parent) {
  // 值类型
  ...
  // 所有支持的类型
  const allTypes = [undefinedTag, nullTag, boolTag, numberTag, stringTag, symbolTag, bigIntTag, arrayTag, objectTag, setTag, mapTag, argTag, regexpTag, dateTag, funcTag, promiseTag, weakMapTag, weakSetTag, errorTag];
  ...
  // 值类型数组
  const valTypes = [undefinedTag, nullTag, boolTag, numberTag, stringTag, symbolTag, bigIntTag];
  // 值类型直接返回
  ...
  forEach(keys || target, (value, key) => {
    ...
    cloneTarget[key] = deepClone(target[key], isFull, cache, target);
  });
...
setTimeout(() => {
  console.log(new dateType.constructor(dateType));
}, 5000);
